<header>
    ...
    <nav class="navbar navbar-expand-md navbar-light fixed-top bg-white">
        <a href="{{ route('frontend.index') }}" class="navbar-brand"><img height="100px"
                                                                          src="{{url('img/frontend/logo_horiz.png') }}"></a>
        ...
        <div class="collapse navbar-collapse justify-content-end" id="navbarSupportedContent">
            <ul class="navbar-nav">
                ...
                @guest
                    <li class="nav-item"><a href="{{route('frontend.auth.login')}}"
                                            class="nav-link {{ active_class(Active::checkRoute('frontend.auth.login')) }}">{{ __('navs.frontend.login') }}</a>
                    </li>

                    @if (config('access.registration'))
                        <li class="nav-item"><a href="{{route('frontend.auth.register')}}"
                                                class="nav-link {{ active_class(Active::checkRoute('frontend.auth.register')) }}">Join
                                Us</a></li>
                    @endif
                @else
                    <li class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuUser">
                            @can('view backend')
                                <a href="{{ route('admin.dashboard') }}"
                                   class="dropdown-item">{{ __('navs.frontend.user.administration') }}</a>
                            @endcan

                            <a href="{{ route('frontend.user.account') }}"
                               class="dropdown-item {{ active_class(Active::checkRoute('frontend.user.account')) }}">{{ __('navs.frontend.user.account') }}</a>
            ...
            @if(Cart::count() == 0)
                <div class="ml-2">
                    <a href="{{ route('frontend.cart') }}" style="color:#2e3192; text-decoration: none"><i
                                class="fas fa-shopping-cart fa-2x ml-2 mr-2"></i>
                        <span class="badge badge-primary mr-2"
                              style="background-color:#2e3192">{{ Cart::count() }}</span></a>
                </div>
            @else
                <div class="ml-2">
                    <a href="{{ route('frontend.cart') }}" style="color:#2e3192; text-decoration: none"><i
                ...
                <ul class="navbar-nav ml-1 mr-1">
                    <li class="nav-item">
                        ...
                        @if($cart_total > $logged_in_user->balance )
                            <span class="text-danger"><i class="fas fa-pound-sign fa-2x pr-1"
                                                         aria-hidden="true"></i>{{ $logged_in_user->balance - $cart_total }}</span>
                        @else
                            <i style="color:#2e3192" class="fas fa-pound-sign fa-2x pr-1"
                               aria-hidden="true"></i>{{ $logged_in_user->balance - $cart_total }}
                        @endif
                        {{--</a>--}}
                    </li>
                </ul>
